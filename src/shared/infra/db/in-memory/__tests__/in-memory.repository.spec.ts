...
describe('InMemoryRepository Unit Tests', () => {
  ...
  test('should bulk insert a entities', async () => {
    const entities = [
      new StubEntity({
        entity_id: new Uuid(),
        name: 'Test',
  ...
  it("should throws error on update when entity not found", async () => {
    ...
    await expect(repo.update(entity)).rejects.toThrow(
      new EntityNotFoundError(entity.entity_id, StubEntity)
    );
  });
  ...
  it("should throws error on delete when entity not found", async () => {
    ...
    await expect(repo.delete(uuid)).rejects.toThrow(
      new EntityNotFoundError(uuid.id, StubEntity)
    );

    await expect(
      repo.delete(new Uuid("9366b7dc-2d71-4799-b91c-c64adb205104"))
    ).rejects.toThrow(
      new EntityNotFoundError("9366b7dc-2d71-4799-b91c-c64adb205104", StubEntity)
    );
  });
